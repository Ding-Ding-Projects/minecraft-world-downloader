...
| 14 | Squirrel.Windows lifecycle | `npx vitest run tests/unit/squirrel.test.ts` | `src/main/squirrel.ts`’s `handleSquirrelEvent()` against every argument Squirrel actually passes: `--squirrel-install` and `--squirrel-updated` spawn `Update.exe --createShortcut` for `Desktop,StartMenu`; `--squirrel-uninstall` removes them; `--squirrel-obsolete` quits without spawning; `--squirrel-firstrun` starts NORMALLY (it is the user’s own launch, not housekeeping); an ordinary launch is untouched; nothing runs off Windows. Critically asserts the quit happens **after** `Update.exe` finishes, never at the moment the event is recognised — quitting early would kill the shortcut creation this exists to perform. | **PASS — 10/10 tests.** Watched red before green: reducing `handleSquirrelEvent` to `return false` (the exact state the application shipped in) fails 7 of the 10. |
| 15 | Shell navigation vs subtitle channels | `npx vitest run tests/unit/shell-subtitle.test.ts` | The invariant whose absence left the application on a permanently blank window: `shell.setSubtitle()` must notify the SUBTITLE channel and never the navigation channel, because `renderActiveScreen` is on the navigation channel and mounts the active screen. Asserts a subtitle change fires zero navigations, that 50 consecutive subtitle pushes still fire zero, that the subtitle channel does fire so the header still updates, that an identical repeated subtitle notifies nothing, and — guarding the guard — that a real `go()` still fires navigation exactly once. | **PASS — 5/5 tests.** Watched red before green: restoring the original `this.emit()` inside `setSubtitle` fails 4 of the 5. |
