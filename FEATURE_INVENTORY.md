...
| 13.6 | A version-controlled vault for a downloaded world: the application creates a local repository for it, and an optional user-initiated publish to a remote. | `features/world-vault` | documented | ⬜ | Publishing is never automatic. A downloaded world can contain another server's builds, player-built structures and container contents, so sending it anywhere is a decision the user makes explicitly, with what is about to leave the machine named before it does. |
| 13.7 | A background runner that commits while the download is in progress, so any moment can be returned to. | `features/world-vault` | documented | ⬜ | Commits on settled writes, never on every filesystem event: a region file is rewritten continuously as chunks stream in, and a snapshot taken mid-write stores a corrupt chunk. Debounced with a quiet period, with the disk cost of retaining history stated plainly before it is switched on and visible while it runs. |
| 13.7a | The vault stores world files in ordinary object storage, and proves the assumption that lets it. | `features/world-vault` | documented | ⬜ | No world file reaches 50 MB, so the large-file transfer path is not required and no large-file filter or pointer storage is introduced. That is a measured property of the data, not a hope: the runner checks each file against the bound before committing it, and a file that ever exceeds it stops the commit and reports the exact path and size rather than quietly storing a heavyweight object. The cost that remains is aggregate history growth across many small files, which retention and pruning address and which no per-object transfer path would have solved. |
| 13.8 | Unlimited undo across that history, where restoring is itself a new commit. | `features/world-vault` | documented | ⬜ | Append-only, exactly as the application's own history behaves: an undo can be undone, and that undo undone in turn. A restore never discards the state it replaced, because a history that can lose the present is not safe to experiment in. |
| 13.9 | An optional map render per commit, so two moments in a world can be compared visually. | `features/world-vault-renders` | documented | ⬜ | A render costs minutes and needs a Java runtime, so it is queued and bounded, never blocking the download or the commit that triggered it. A commit with no render says so rather than showing a neighbour's. |
| 13.10 | Chunk operations driven from the rendered map: select a chunk, copy it to another coordinate, or remove it. | `features/world-vault-edit` | documented | ⬜ | A copy rewrites the chunk's own coordinates and the positions inside its block entities and entities — copying the bytes alone produces a chunk that renders in the wrong place or is rejected outright. A removal clears the region's sector entry. Every edit is a commit, so the unlimited undo covers edits as well as downloads, and editing a region the downloader is actively writing is refused rather than raced. |
| 13.11 | An end-to-end harness that proves the downloader works: a real server, bots driven through the proxy, and the saved world checked on disk afterwards. | `features/downloader-e2e` | documented | ⬜ | The one check that tests the product rather than a part of it. A bot walking through the proxy is what makes the server stream chunks, so the run exercises the real protocol path rather than a fixture. Success is measured by reading the region files back and counting the chunks actually saved — a run that completes without producing a world has not passed, however green it looked. |
...
| 14.6 | An executable negative regression that removes one asserted item at a time from this inventory and must turn red. | `scripts/check-inventory-negative.mjs` | n/a | ⬜ | Lives beside `check-inventory.mjs` rather than under `app/tests`: the inventory it guards spans the whole repository (build scripts, the release workflow, the site, every feature directory), not only application code, so a repository-wide guard runs as a standalone script rather than inside the app's own test runner. Runs eight mutations — one at a time, each against a throwaway mirror, never the real tree — covering a deleted feature directory, a deleted doc article, a deleted named core module, a neutralised default export, a blanked status mark, an unlisted feature directory, a removed site, and a removed build script. Each must independently turn the guard red; the positive control reports `INCONCLUSIVE` rather than a false pass if the unmutated baseline is not already green. Its own `MIRROR_PATHS` used to name two individual scripts instead of the whole `scripts/` directory, so any row (this one included) naming a different script made the unmutated mirror fail for a reason no mutation caused — fixed to mirror the directory. Verified 2026-08-13: `PASS — 8 caught, 0 missed, 0 skipped.` |
